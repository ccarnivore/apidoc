...
function createOutputFiles(api) {
    ...
    if (app.options.zip) {
        app.log.info('creating zip file of contents');
        zipdir(app.options.dest, { saveTo: app.options.dest + 'apiDoc.zip', filter: (path, stat) => !/\.zip$/.test(path) }, () => {});
    }

    if (fs.existsSync(app.options.dest + 'index.php') && fs.existsSync(app.options.dest + 'index.html')) {
        fs.unlink(app.options.dest + 'index.html');
    }
}
